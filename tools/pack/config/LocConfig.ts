...
export function parseLocConfig(key: string, value: string): ConfigValue | null | undefined {
    ...
    try {
        if (stringKeys.includes(key)) {
            ...
            const locId = locName === 'null' ? 0xffff : LocPack.getByName(locName);
            if (locId === -1) {
                return null;
            }

            return (locId << 16) | index;
        }
        else if (key.startsWith('model')) {
            const args = value.split(',');
            const model = ModelPack.getByName(args[0]);
            if (model === -1) {
                return null;
            }

            let shape: number = 0xffff;
            if (args.length > 1) {
                const shapeKey = args[1];
                shape = LocShape[shapeKey as keyof typeof LocShape];
            }
            return model << 16 | shape;
        }
        ...
        console.error('Error parsing loc config: key='+ key +', value='+ value +', err='+ err);
        throw err;
    }
}

export function packJs5LocConfigs(configs: Map<string, ConfigLine[]>, archive: Js5Archive): void {
        ...
        try {
            ...
            for (let j = 0; j < config.length; j++) {
                ...
                if (key === 'name') {
                    ...
                    const modelId = packed >> 16;
                    const shape = packed & 0xffff;
                    models.push({
                        packedModel: modelId << 16 | index,
                        shape
                    });
                } else if (key.startsWith('recol')) {
                    const index = parseInt(key[5]) - 1;
                    ...
                    const packed = value as number;
                    multiLocs.push(packed);
                }
            }

            if ((multiVarp !== -1 || multiVarbit !== -1) && multiLocs.length > 0) {
                client.p1(77);

                client.p2(multiVarbit);
                client.p2(multiVarp);

                const locs = multiLocs.sort((a, b) => {
                    const indexA = a & 0xffff;
                    const indexB = b & 0xffff;
                    return indexA - indexB;
                });

                client.p1(locs.length - 1);

                locs.forEach(loc => {
                    const locId = loc >> 16;

                    client.p2(locId == 0xffff ? -1 : locId);
                });
            }

            if (models.length > 0) {
                const opcode = models.every(model => model.shape == 0xffff) ? 5 : 1;

                const sortedModels = models.sort((a, b) => {
                    const indexA = a.packedModel & 0xffff;
                    const indexB = b.packedModel & 0xffff;
                    return indexA - indexB;
                });

                client.p1(opcode);

                client.p1(sortedModels.length);

                sortedModels.forEach(model => {
                    client.p2(model.packedModel >> 16);

                    if (model.shape != 0xffff) {
                        client.p1(model.shape);
                    }
                });
            }
            ...
            if (name === null && active !== 0) {
                ...
                if (active === -1) {
                    for (let k = 0; k < models.length; k++) {
                        if (models[k].shape === LocShape.centrepiece_straight) {
                            // the presence of any centrepiece_straight shape means we have to transmit a name
                            shouldTransmit = true;
                            break;
                        }
                    }
            ...
            archive.writeFile(6, i, client.data.subarray(0, client.pos));
        }
        catch (err) {
            const debugname = LocPack.getById(i);
            const config = configs.get(debugname)!;
            console.error(`failed to pack loc ${i}, debugname=${debugname}, configs=${JSON.stringify(config)}`, err);
            throw err;
        }
    }
}

export function packLocConfigs(configs: Map<string, ConfigLine[]>): { server: PackedData } {
